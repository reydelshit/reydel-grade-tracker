...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.course = self.ui.comboBox_2
        ...
        # Populate the initial data in the table and subject/course dropdowns
        self.search_info()
        self.update_subject_course()
    ...
    def update_info(self):
        ...
        if new_student_info["student_id"]:
            update_result = self.db.update_info(
                ...
                subject=new_student_info["subject"],
                course=new_student_info["course"]
            )
    ...
    def select_info(self):
        ...
        if select_row != -1:
            ...
            subject = self.result_table.item(select_row, 4).text().strip()
            course = self.result_table.item(select_row, 3).text().strip()
            grade = self.result_table.item(select_row, 5).text().strip()
    ...
    def update_subject_course(self):
        # Function to populate the subject and course dropdowns
        subject_result = self.db.get_all_subjects()
        course_result = self.db.get_all_cities()
        ...
        if len(course_list) > 1:
            self.course.addItems(course_list)
